            ...
            <#list productList as x>
                <li id="p-${x.id}">
                    <a href="/show?id=${x.id}" class="link">
                        ...
                        <h3>${x.title}</h3>
                        <div class="price"><span class="v-unit">¥</span><span class="v-value">${x.price}</span></div>
                        <#if user?? && user.userType==0 && x.buy == 1 ><span class="had"><b>已购买</b></span></#if>
                        <#if user?? && user.userType==1 && x.sell == 1 ><span class="had"><b>已售出</b></span></#if>
                    </a>
                    <#if user?? && user.userType==1 && x.sell == 0><span class="u-btn u-btn-normal u-btn-xs del" data-del="${x.id}">删除</span></#if>
                </li>
            </#list>
        </#if>
        </ul>
